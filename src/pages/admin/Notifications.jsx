...
const Notifications = () => {
  ...
  async function fetchNotifications() {
    // Remplacez '1' par l'ID de l'admin connecté si besoin
  ...
  const clearAll = async () => {
    await supabase.from('notifications').delete().neq('id', 0); // supprime tout
    setNotifications([]);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
            ...
            >
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <span className="mt-1 text-xs text-gray-500">{new Date(notification.created_at).toLocaleString()}</span>
                </div>
              </div>
        ...
        {notifications.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-lg bg-white py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900">{t('notifications.no_notifications')}</h3>
            <p className="text-gray-500">{t('notifications.all_caught_up')}</p>
          </div>
        )}
      </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-md">
        <h2 className="mb-4 text-lg font-medium">{t('notifications.settings_title', 'Notification Settings')}</h2>
        <div className="space-y-4">
          {NOTIF_SETTINGS.map((setting, index) => (
            <div key={index} className="flex items-center justify-between rounded-lg border border-gray-200 p-4">
              <div>
                <h3 className="font-medium text-gray-900">{t(`notifications.${setting.label}`)}</h3>
                <p className="text-sm text-gray-500">{t(`notifications.${setting.desc}`)}</p>
              </div>
